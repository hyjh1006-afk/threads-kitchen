...
> **2026-08-11 계정 이전:** 기존 `@diet_zze` 비활성화 후 새 `@pparkzze`로 이전 준비 중.
> `m01~m15` 과거 게시물을 하루 1개씩 먼저 재생하고, 완료 후 새 메뉴로 전환한다.
> GitHub Actions는 새 계정의 `THREADS_USER_ID`와 `THREADS_ACCESS_TOKEN` 검증 전까지 수동 비활성화 상태다.
